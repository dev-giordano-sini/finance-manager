...
public final class UserService implements UserUseCase {
    ...
    public UserService(CurrentActorPort actor, UserPort users) {
        resolver = new UserResolver(actor, users);
    }
    public CurrentUser current() {
        User u = resolver.current();
        return new CurrentUser(u.name(), u.surname(), u.email());
    }
}
