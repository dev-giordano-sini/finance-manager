...
public class SecurityConfig {
    @Bean
    UserDetailsService userDetailsService(UserOutputPort repository, RoleOutputPort roleRepository) {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http, JwtAuthenticationFilter filter, ObjectMapper mapper) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable).cors(cors -> {})
                ...
                .exceptionHandling(errors -> errors
                        .authenticationEntryPoint((request, response, ex) -> writeProblem(response, mapper, 401, "Unauthorized", "Authentication is required"))
                        .accessDeniedHandler((request, response, ex) -> writeProblem(response, mapper, 403, "Forbidden", "Access is denied")))
