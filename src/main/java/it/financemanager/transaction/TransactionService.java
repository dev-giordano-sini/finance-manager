...
public class TransactionService implements TransactionUseCase {
    private final TransactionStore repository;
    private final CategoryUseCase categories;
    private final CurrentUserProvider currentUser;

    public TransactionService(TransactionStore repository,
    ...
    public PageResult<TransactionResponse> list(LocalDate from, LocalDate to,
        ...
        PageResult<Transaction> page = repository.findAllByUserIdAndDateBetween(
                currentUser.get().getId(), from, to, pageable);
        return new PageResult<>(page.content().stream().map(this::map).toList(),
    ...
    public TransactionResponse create(SaveTransactionCommand request) {
        ...
        return map(repository.save(new Transaction(user, category, request.type(),
    ...
    public TransactionResponse update(Long id, SaveTransactionCommand request) {
        User user = currentUser.get();
        Transaction value = find(id, user.getId());
        Category category = categories.find(request.categoryId(), user.getId());
        value.update(category, request.type(), request.amount(), request.date(),
                clean(request.description()));
    ...
    private Transaction find(Long id, Long userId) {
