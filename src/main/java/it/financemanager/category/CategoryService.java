...
public class CategoryService implements CategoryUseCase {
    ...
    public CategoryService(CategoryStore repository,
    ...
    public List<CategoryResponse> list() {
        ...
        return repository.findAllByUserIdOrderByNameAsc(user.getId())
                .stream()
                .map(this::map)
                .toList();
    }
    ...
    public CategoryResponse create(SaveCategoryCommand request) {
        ...
        return map(repository.saveAndFlush(
                new Category(user, name, request.color().toUpperCase())));
    }
    ...
    public CategoryResponse update(Long id, SaveCategoryCommand request) {
        ...
        if (!category.getName().equalsIgnoreCase(name) &&
                repository.existsByUserIdAndNameIgnoreCase(user.getId(), name))
            throw new ConflictException("Category name already exists");
        category.update(name, request.color().toUpperCase());
        return map(repository.saveAndFlush(category));
    }
