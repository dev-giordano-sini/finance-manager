...
public class DashboardService implements DashboardUseCase {
    ...
    public DashboardService(TransactionStore transactions,
    ...
    public DashboardResponse get(LocalDate from, LocalDate to) {
        ...
        return new DashboardResponse(
                resolvedFrom, resolvedTo, income, expenses, income.subtract(expenses),
                values.size(), categoryExpenses(values, expenses),
    ...
    private List<DashboardResponse.CategoryExpense>
    categoryExpenses(List<Transaction> values, BigDecimal totalExpenses) {
        ...
        values.stream()
                ...
                .forEach(
                        value
                                ...
                                (id, current)
                                        -> current == null
                                        ? new CategoryTotal(value.getCategory().getName(),
                                        value.getCategory().getColor(),
                                        value.getAmount())
        ...
        return totals.entrySet()
                .stream()
                .map(entry
                        -> new DashboardResponse.CategoryExpense(
                        entry.getKey(), entry.getValue().name(),
                        entry.getValue().color(), entry.getValue().amount(),
    ...
    private List<DashboardResponse.DailyCashFlow>
    dailyCashFlow(List<Transaction> values, LocalDate from, LocalDate to) {
        Map<LocalDate, BigDecimal[]> totals = new TreeMap<>();
        values.forEach(value -> {
            BigDecimal[] day = totals.computeIfAbsent(
                    value.getDate(), ignored -> new BigDecimal[]{ZERO, ZERO});
        ...
        from.datesUntil(to.plusDays(1)).forEach(date -> {
            BigDecimal[] day =
                    totals.getOrDefault(date, new BigDecimal[]{ZERO, ZERO});
            result.add(new DashboardResponse.DailyCashFlow(date, day[0], day[1],
                    day[0].subtract(day[1])));
        });
        return result;
    }
    ...
    private TransactionResponse map(Transaction transaction) {
        return new TransactionResponse(
                transaction.getId(), transaction.getCategory().getId(),
                transaction.getCategory().getName(), transaction.getType(),
