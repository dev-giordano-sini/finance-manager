...
class TransactionalAuthUseCase implements AuthUseCase {
    ...
    TransactionalAuthUseCase(UserStore users, PasswordHasher hasher, CredentialAuthenticator authenticator,
                             AccessTokenIssuer tokens, RoleStore roles) {
        delegate = new AuthService(users, hasher, authenticator, tokens, roles);
    }
    @Override @Transactional public AuthResponse register(RegisterCommand command) { return delegate.register(command); }
    @Override public AuthResponse login(LoginCommand command) { return delegate.login(command); }
}
