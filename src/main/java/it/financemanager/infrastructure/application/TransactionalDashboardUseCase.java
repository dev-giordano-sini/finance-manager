...
class TransactionalDashboardUseCase implements DashboardUseCase {
    ...
    TransactionalDashboardUseCase(TransactionStore transactions, CurrentUserProvider user, Clock clock) { delegate = new DashboardService(transactions, user, clock); }
    @Override public DashboardResponse get(LocalDate from, LocalDate to) { return delegate.get(from, to); }
}
