...
import it.financemanager.transaction.SaveTransactionCommand;
import it.financemanager.transaction.TransactionResponse;
import it.financemanager.transaction.TransactionUseCase;
...
public class TransactionController {
    ...
    list(@RequestParam(defaultValue = "1900-01-01")
         @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
         @RequestParam(defaultValue = "2999-12-31")
         @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to,
         @PageableDefault(size = 20, sort = "date",
                 direction = Sort.Direction.DESC) Pageable pageable) {
        String sortBy = pageable.getSort()
                .stream()
                .findFirst()
                .map(Sort.Order::getProperty)
                .orElse("date");
        PageQuery.Direction direction = pageable.getSort()
                .stream()
                .findFirst()
        ...
        return service.list(from, to,
                new PageQuery(pageable.getPageNumber(),
    ...
    @PutMapping("/{id}")
    TransactionResponse update(@PathVariable Long id,
    ...
    private SaveTransactionCommand command(TransactionRequest value) {
        return new SaveTransactionCommand(value.categoryId(), value.type(),
                value.amount(), value.date(),
                value.description());
    }
}
