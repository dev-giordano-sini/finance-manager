...
public class CategoryController {
    ...
    @PostMapping @ResponseStatus(HttpStatus.CREATED) CategoryResponse create(@Valid @RequestBody CategoryRequest request) { return service.create(command(request)); }
    @PutMapping("/{id}") CategoryResponse update(@PathVariable Long id, @Valid @RequestBody CategoryRequest request) { return service.update(id, command(request)); }
    @DeleteMapping("/{id}") @ResponseStatus(HttpStatus.NO_CONTENT) void delete(@PathVariable Long id) { service.delete(id); }
    private SaveCategoryCommand command(CategoryRequest value) { return new SaveCategoryCommand(value.name(), value.color()); }
}
