...
public class JwtAdapter implements TokenPort {
    ...
    public JwtAdapter(JwtProperties p) {
        this.p = p;
        key = Keys.hmacShaKeyFor(p.secret().getBytes(StandardCharsets.UTF_8));
    }
    public String generate(String s) {
        ...
        return Jwts.builder()
            ...
            .expiration(Date.from(n.plus(p.expiration())))
            .signWith(key)
            .compact();
    }
    public String subject(String t) {
        return Jwts.parser().verifyWith(key).build().parseSignedClaims(t).getPayload().getSubject();
    }
    public long expiresInSeconds() {
        return p.expiration().toSeconds();
    }
}
