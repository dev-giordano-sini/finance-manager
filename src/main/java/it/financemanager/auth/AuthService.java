...
public class AuthService implements AuthUseCase {
    ...
    public AuthService(UserStore users, PasswordHasher passwordHasher,
    ...
    public AuthResponse register(RegisterCommand command) {
        ...
        Role user =
                roles.findByCode(BaseRole.ROLE_USER.getRole())
                        .orElseThrow(()
                                -> new IllegalStateException(
                                "Default USER role is not configured"));
        users.saveAndFlush(new User(email, passwordHasher.hash(command.password()),
                command.name().trim(), command.surname(),
                user));
        return token(email);
    }
    ...
    private AuthResponse token(String email) {
        return new AuthResponse(tokens.issue(email), "Bearer",
